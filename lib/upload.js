...
import { sendChecksumHeader } from "./checksum";
...
class Upload {
  ...
  _addChunkToRequest(xhr) {
    ...
    this._source.slice(start, end, (err, value, complete) => {
      ...
      const send = () => {
        xhr.send(value === null ? undefined : value);
        this._emitProgress(this._offset, this._size);
      }

      if (this.options.checksumAlgorithm !== null) {
        console.info(`sending checksum header for start ${start} end ${end} size: ${value ? value.size: 0}` )
        sendChecksumHeader(this.options.checksumAlgorithm, xhr, value, send);
      } else {
        send();
      }
    });
